...
for i in np.arange(12)[:]:
	...
	temp_3D_mean = np.full((5000,y.shape[0]-1,x.shape[0]-1),np.nan)
	saln_3D_mean = np.full((5000,y.shape[0]-1,x.shape[0]-1),np.nan)
	temp_3D_stdv = np.full((5000,y.shape[0]-1,x.shape[0]-1),np.nan)
	saln_3D_stdv = np.full((5000,y.shape[0]-1,x.shape[0]-1),np.nan)
	...
	for ii in np.arange(place2.statistic.T.shape[0]):
		for iii in np.arange(place2.statistic.T.shape[1]):
			...
			if place2.statistic.T[ii,iii] > 3:
				...
				for iv in np.arange(temp_month.shape[1]):

					#Determine if there's enough measurements to proceed, temperature
					#If within surface, within bottom, or between 
					if iv < depth_window:
						temp_slice = temp_month[cast_isolate][:,:iv+depth_window+1]

					elif iv > temp_month.shape[1] - depth_window:
						temp_slice = temp_month[cast_isolate][:,iv-depth_window:]
					else:
						temp_slice = temp_month[cast_isolate][:,iv-depth_window:iv+depth_window+1]
					...
					#If within surface, within bottom, or between 
					if iv < depth_window:
						saln_slice = saln_month[cast_isolate][:,:iv+depth_window+1]

					elif iv > saln_month.shape[1] - depth_window:
						saln_slice = saln_month[cast_isolate][:,iv-depth_window:]
					else:
						saln_slice = saln_month[cast_isolate][:,iv-depth_window:iv+depth_window+1]
					...
					print('Depth: '+str(iv)+', Y: '+str(ii)+', X: '+str(iii)+', Month: '+str(i+1))
	...
	#Create dimensions
	level = nc_out.createDimension('level', 5000) 
